...
- 화면/라우트/파일 책임 분리 관련 작업 시 아래 문서를 **항상 먼저 참조**한다.
  - `docs/VIEW_ROUTE_AND_FILE_HIERARCHY.md`
- 구조 변경(파일 분리, 네이밍 변경, 라우트 추가/이동, part 분리/병합, 동기화 책임 이동)이 발생하면
  - 작업 완료 시점에 `docs/VIEW_ROUTE_AND_FILE_HIERARCHY.md`를 **반드시 갱신**한다.
  - 필요 시 `docs/PLAN_BASIC_STRUCTURE.md`, `todo.md`도 함께 동기화한다.
- 문서 갱신은 일회성이 아니라, 구조 변경이 누적되면 **주기적으로 최신 상태를 반영**한다.
  - 기준: 구조 관련 PR/작업 단위가 끝날 때마다 점검 및 업데이트.
