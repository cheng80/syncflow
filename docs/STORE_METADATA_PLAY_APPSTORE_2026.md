...
이 문서는 2026-03-05 기준으로 **Google Play / Apple 공식 문서 요구사항 + 현재 앱 구현 상태**를 반영한 등록용 메타데이터입니다.  
목표는 아래 2가지입니다.
...
- 6자리 초대 코드로 멤버 참가
- 템플릿 기반 빠른 보드 생성
- 멘션(@user)·담당자 변경 푸시 알림
- 앱 실행 중(포그라운드) 로컬 알림 표시
- 푸시 탭 시 해당 보드/카드 화면으로 바로 이동(딥링크)
- 라이트/다크/시스템 테마
...
- 알림 권한(선택): 멘션/담당자 변경 푸시 수신에 필요합니다.
```
...
- Mention and assignee-change push notifications
- Foreground local notifications while the app is open
- Push tap deep link to the target board/card
...
- Notifications (optional): required for mention/assignee push alerts.
```
...
- Data collected: `Yes` (이메일, 보드·카드 데이터, 멘션/담당자 알림 처리용 푸시 토큰)
...
- 템플릿 기반 빠른 보드 생성
- 멘션(@user) 및 담당자 변경 푸시 알림
- 앱 실행 중(포그라운드) 로컬 알림 표시
- 푸시 탭 시 해당 보드/카드로 바로 이동
- 테마, 다국어 지원
...
- 총 10장(스토어 설명 흐름: 로그인/보드 목록 -> 보드 상세 -> 카드 편집/멘션 -> 담당자 지정 -> 푸시 수신/딥링크)
...
- [ ] 최신 UI 기준 스크린샷 교체 (멘션/담당자/푸시 딥링크 흐름 반영)
